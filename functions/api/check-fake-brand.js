...
export async function onRequest(context) {
    ...
    try {
        ...
        // 🚨 優化提示詞：明確告訴 AI 不要把正常的促銷當成詐騙
        const prompt = `你是一位嚴格的資安分析師。請閱讀以下的網頁內容，進行判斷：
        1. 如果網頁「明確」冒用特定知名品牌（如：中國信託、台灣大哥大、中華郵政等），請直接輸出該「品牌名稱」。
        2. 如果這是一般的「購物網站」、「一頁式廣告」、「合法電商」，且沒有明確聲稱自己是知名大廠，或是你無法 100% 確定，請嚴格輸出「Unknown」。絕對禁止胡亂猜測銀行或品牌，也絕對不可以因為有打折促銷就當作詐騙。
        3. 如果內容包含強烈的通用詐騙特徵（如：保證獲利、高薪免經驗代工、要求加 LINE 領飆股、解除分期付款詐騙等），請輸出「Generic_Scam」。
