...
同一篇文章涵蓋不同階段時，可加上 `alternativeRequired`（最多四組，
每組至少兩個不同且已定義的概念）。任一替代組合須在八行 OCR 範圍內、
同一段連續可讀文字中完整成立；不跨低信心缺口拼湊情境，仍保留每行
80 分的門檻。原本 `required` 條件仍可獨立匹配。

例如 `vote-account` 原本的「比賽投票＋登入／驗證碼」之外，新增
「投票＋作品／繪畫比賽＋請託幫忙」三概念，用於尚未出現索取帳號的前期提醒。
依據為已查閱的 https://www.mygopen.com/2026/08/vote-scam.html，文章亦描述
先向親友請託幫作品投票的階段。這只推薦相關閱讀，不改動風險分數，也不代表
辨識到了登入、驗證碼或已驗證截圖內網址。正常比賽仍可能收到相關閱讀。

2026-09-15 本機實圖回歸：原規則無推薦；修正後成功顯示上述文章。
實測整體 OCR 信心 79，但關鍵文字行均超過 80，因此沿用逐行品質判斷，
不降低門檻。圖片中的連結未被可靠辨識；圖片未上傳，測試未呼叫雲端 AI。
儲存庫只保留去識別化的合成文字測試，不收錄原始截圖。
