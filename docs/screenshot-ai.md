...
2. Send locally extracted URLs through the existing main URL scanner. The image content remains **unknown**, not safe. The main result provides an **分析圖片內容** action for a separate content assessment. Multiple extracted destinations remain selectable; chat scans its extracted destinations in sequence.
3. If there is no locally usable URL, normalize the screenshot in the browser and call `/api/cf-vision`. PNG/JPEG/WebP uploads are limited to 3 MiB; normalized images have a maximum 2400-pixel long edge and 20-million-pixel source limit. Canvas re-encoding removes source metadata. Do not upload sensitive material unnecessarily.
