...
test('手機版聊天小幫手浮動泡泡支援拖曳、邊界限制與拖曳後防誤觸', () => {
    ...
    assert.match(source, /onPointerMove=\{handleBubblePointerMove\}/);
    assert.match(source, /setPointerCapture/);
    assert.match(source, /window\.innerWidth - drag\.width/);
    assert.match(source, /window\.innerHeight - drag\.height/);
    assert.match(source, /bubbleWasDraggedRef\.current/);
    assert.match(source, /touch-none select-none md:touch-auto/);
});
...
test('油跡可循食安資訊平台應視為可信台灣服務且不被誤判為高風險', () => {
    ...
    assert.ok(riskConfig.trustedTaiwanServiceDomains.includes('oiltrace.tw'));
    assert.equal(matchesDomainList(hostname, whitelist), true);
    assert.equal(isTrustedTaiwanServiceDomain(hostname), true);
    assert.equal(isVerifiedSafeRootDomain(hostname), true);
    assert.equal(shouldSkipAiBrandAnalysis(hostname), true);
    ...
    assert.equal(override.googleFlaggedForRisk, false);
    assert.equal(override.riskScore, 0);
    assert.equal(isVerifiedSafeRootDomain('oiltrace.tw.evil.shop'), false);
...
test('奧斯汀美語官方網域應視為可信台灣教育服務且不被誤判為高風險', () => {
    ...
    const jobSignals = analyzeJobTaskScamSignals({
        url: 'https://austinenglish.com.tw/teacher-training/',
        html: '奧斯汀美語師資培訓與徵才，完成培訓後有機會加入教學團隊。總部專線 03-357-0230。'
    });

    assert.ok(riskConfig.trustedTaiwanServiceDomains.includes('austinenglish.com.tw'));
    assert.equal(matchesDomainList(hostname, whitelist), true);
    assert.equal(isTrustedTaiwanServiceDomain(hostname), true);
    assert.equal(isVerifiedSafeRootDomain(hostname), true);
    assert.equal(isVerifiedSafeRootDomain(branchHostname), true);
    assert.equal(shouldSkipAiBrandAnalysis(hostname), true);
    assert.equal(jobSignals.matched, false);
    assert.equal(override.hasTrustedAllowlistOverride, true);
    assert.equal(override.blocklistListedForRisk, false);
    assert.equal(override.googleFlaggedForRisk, false);
    assert.equal(override.riskScore, 0);
    assert.equal(isVerifiedSafeRootDomain('austinenglish.com.tw.evil.shop'), false);
...
test('富邦公益大使官方次網域不應因投票、註冊、手機驗證與抽獎語意誤判為高風險', () => {
    ...
    const html = `
        <html lang="zh-TW">
            <head>
                <title>富邦公益大使</title>
                <meta name="description" content="富邦公益大使活動網站">
            </head>
            <body>
                <main>
                    <h1>2026 富邦公益大使計畫</h1>
                    <p>本活動由富邦慈善基金會主辦，邀請富邦媒體科技共同提供公益資源。</p>
                    <p>請先註冊會員並使用手機驗證碼登入，即可參與公益提案網路票選及抽獎活動。</p>
                    <form>
                        <input name="mobile" placeholder="手機號碼">
                        <input name="verification_code" placeholder="手機驗證碼">
                        <button>投票登入</button>
                    </form>
                    <a href="/activity">活動緣起</a><a href="/activity/about">活動辦法</a>
                    <a href="/project/index/ALL">票選瀏覽</a><a href="/subsidy">補助名單</a>
                    <a href="https://www.fuboncharity.org.tw/chinese/">富邦慈善基金會</a>
                </main>
            </body>
        </html>`;
    const sanitized = sanitizeUrlForRiskScoring(rawUrl);
    ...
    assert.ok(riskConfig.trustedTaiwanServiceDomains.includes('fuboncharity.org.tw'));
    assert.ok(riskConfig.protectedBrands.find(brand => brand.name === '富邦銀行').domains.includes('fuboncharity.org.tw'));
    ...
    assert.equal(isTrustedCoBrandCampaignHost(parsed.hostname, '富邦慈善基金會'), true);
    assert.equal(isTrustedCoBrandCampaignHost(parsed.hostname, '台北富邦銀行'), true);
    assert.equal(fakeFubon.matched, true);
    assert.equal(fakeFubon.brandName, '富邦銀行');
    assert.equal(override.hasTrustedAllowlistOverride, true);
    assert.equal(override.riskScore, 0);
    assert.equal(isVerifiedSafeRootDomain('fuboncharity.org.tw.evil.shop'), false);
    assert.equal(isVerifiedSafeRootDomain('fake-fuboncharity.org.tw'), false);
...
test('全球頂級可信根網域即使白名單載入失敗也應保留 root override', () => {
    ...
    assert.equal(isVerifiedSafeRootDomain('store.dji.com'), true);
    ...
    assert.equal(isVerifiedSafeRootDomain('dji.com.evil.shop'), false);
...
test('大型電商根網域子網域應直接視為可信 allowlist 並維持低風險', () => {
    const trustedEcUrls = [
        ...
        'https://store.dji.com/product/dji-mini-5-pro?utm_source=google&gclid=abc123'
    ];
...
test('聖弘紙藝官方網站不應因 LINE 訂購與免運文字誤判為高風險', () => {
    ...
    const whitelist = JSON.parse(fs.readFileSync(path.join(repoRoot, 'whitelist.json'), 'utf8')).domains;
    const pageText = `
        <title>紙紮、紙紮屋、紙紮車－聖弘紙藝</title>
        <meta name="description" content="各式品項紙紮專賣店，全台免運配送與線上代燒服務">
        <link rel="canonical" href="https://shenghongpaper.com/">
        <h1>聖弘紙藝</h1>
        <p>經營紙紮逾 40 年，提供紙紮屋、紙紮車、庫錢靈厝到客製化款式等各式喪用紙品。</p>
        <p>從選款、製作到全台配送一手安排，加LINE訂購，全台免運。</p>
        <a href="/paper-house/">紙紮屋</a><a href="/paper-car/">紙紮車</a>
        <a href="/custom/">客製紙紮</a><a href="/reviews/">顧客評價</a>
        <a href="/about/">關於我們</a><a href="/contact/">聯絡我們</a>
        <a href="https://lin.ee/shenghong">加LINE訂購</a>
        <p>付款方式、配送方式、退貨政策、隱私權政策</p>
        <footer>Copyright 2024 聖弘紙藝</footer>
    `;
    const shoppingSignals = analyzeShoppingScamSignals({ html: pageText, url: rawUrl });
    ...
    const scanData = enforceFinalRiskConsistency({
        ...
        checks: {
            ...
            domainAnalysis: { status: 'safe', details: '已驗證聖弘紙藝官方電商根網域：shenghongpaper.com' }
        }
    });
    const brandApiSource = fs.readFileSync(path.join(repoRoot, 'functions/api/check-fake-brand.js'), 'utf8');

    assert.equal(parts.registrableDomain, 'shenghongpaper.com');
    assert.ok(riskConfig.trustedEcommerceRootDomains.includes('shenghongpaper.com'));
    assert.ok(matchesDomainList(parsed.hostname, whitelist));
    assert.equal(isTrustedEcommerceDomain(parsed.hostname), true);
    assert.equal(isVerifiedSafeRootDomain(parsed.hostname, []), true);
    assert.equal(shouldSkipAiBrandAnalysis(parsed.hostname, []), true);
    assert.equal(isTrustedCoBrandCampaignHost(parsed.hostname, '聖弘紙藝'), true);
    assert.equal(shoppingSignals.matched, true);
    assert.equal(shoppingSignals.hasLineContactSignal, true);
    assert.equal(ecommerceSignals.matched, false);
    assert.equal(pageBrandSignals.matched, false);
    assert.equal(fakeShengHong.matched, true);
    assert.equal(fakeShengHong.brandName, '聖弘紙藝');
    assert.equal(override.hasTrustedAllowlistOverride, true);
    assert.equal(override.riskScore, 0);
    assert.equal(scanData.riskScore, 0);
    assert.equal(scanData.summaryReasons, undefined);
    assert.equal(isVerifiedSafeRootDomain('shenghongpaper.com.evil.shop'), false);
...
test('DJI 官方 Store 子網域應視為可信全球品牌商城', () => {
    ...
    const whitelist = JSON.parse(fs.readFileSync(path.join(repoRoot, 'whitelist.json'), 'utf8')).domains;
    const pageText = `
        <title>DJI Store - Camera Drones, Gimbals and Accessories</title>
        <meta name="description" content="DJI Official Online Store provides official products and comprehensive services">
        <link rel="canonical" href="https://store.dji.com/product/dji-mini-5-pro">
        <a href="/cart">Shopping Cart</a><a href="/checkout">Checkout</a>
        <button>Add to Cart</button><button>Buy Now</button>
        <p>DJI Store offers official products, support, accessories, services and up to 30-day returns.</p>
        <p>Payment 付款方式, shipping 配送方式, returns 退換貨政策, privacy policy 隱私權政策 and 服務條款 are available.</p>
        <a href="mailto:support@dji.com">Support</a>
    `;
    const ecommerce = analyzeEcommerceTrustSignals({ url: sanitized.href, html: pageText });
    const shoppingSignals = analyzeShoppingScamSignals({ html: pageText, url: sanitized.href });
    const override = applyTrustedAllowlistRiskOverride({
    ...
    const scanData = enforceFinalRiskConsistency({
        ...
        checks: {
            shoppingScam: { status: shoppingSignals.matched ? 'danger' : 'info' },
            ecommerceValidation: { status: ecommerce.matched ? 'safe' : 'unknown' },
            domainAnalysis: { status: 'safe', details: '受信賴 DJI 官方商城網域：dji.com' }
        }
    });
    const brandApiSource = fs.readFileSync(path.join(repoRoot, 'functions/api/check-fake-brand.js'), 'utf8');

    assert.equal(sanitized.href, 'https://store.dji.com/product/dji-mini-5-pro');
    assert.deepEqual(sanitized.removedTrackingParams.sort(), ['gclid', 'utm_source'].sort());
    assert.equal(parts.registrableDomain, 'dji.com');
    assert.ok(riskConfig.trustedGlobalDomains.includes('dji.com'));
    assert.ok(riskConfig.trustedEcommerceRootDomains.includes('dji.com'));
    assert.ok(matchesDomainList(parsed.hostname, whitelist));
    assert.equal(isTrustedGlobalDomain(parsed.hostname), true);
    assert.equal(isTrustedEcommerceDomain(parsed.hostname), true);
    assert.equal(isVerifiedSafeRootDomain(parsed.hostname, []), true);
    assert.equal(shouldSkipAiBrandAnalysis(parsed.hostname, []), true);
    assert.equal(ecommerce.matched, true);
    ...
    assert.equal(scanData.summaryReasons, undefined);
    assert.equal(isVerifiedSafeRootDomain('dji.com.evil.shop'), false);
    assert.equal(isVerifiedSafeRootDomain('fake-dji.com'), false);
...
test('chesapeakeexpo.com 憑證與跨網域內容不一致應列為人工確認高風險', () => {
    const hostname = 'chesapeakeexpo.com';
    const whitelist = JSON.parse(fs.readFileSync(path.join(repoRoot, 'whitelist.json'), 'utf8')).domains;
    const isConfirmedScam = matchesDomainList(hostname, riskConfig.confirmedScamDomains);
    const scanData = enforceFinalRiskConsistency({
        ...
        checks: {
            ...
            domainAnalysis: {
                status: isConfirmedScam ? 'danger' : 'safe',
                details: '網域、HTTPS 憑證與跨網域轉址內容不一致，請勿輸入個資、帳密或付款資料'
            }
        }
    });

    assert.equal(isConfirmedScam, true);
    assert.equal(matchesDomainList('shop.chesapeakeexpo.com', riskConfig.confirmedScamDomains), true);
    assert.equal(matchesDomainList('chesapeakeexpo.com.safe.example', riskConfig.confirmedScamDomains), false);
    assert.equal(matchesDomainList('fake-chesapeakeexpo.com', riskConfig.confirmedScamDomains), false);
    assert.equal(matchesDomainList(hostname, whitelist), false);
    assert.equal(scanData.riskScore, 100);
    assert.ok(scanData.summaryReasons.includes('人工確認詐騙網域'));
});

test('shopkitchenette.com 到期後重新註冊並更換基礎設施，應列為人工確認高風險', () => {
    const hostname = 'shopkitchenette.com';
    const whitelist = JSON.parse(fs.readFileSync(path.join(repoRoot, 'whitelist.json'), 'utf8')).domains;
    const registrationDate = new Date('2026-07-16T08:20:26Z');
    const checkedAt = new Date('2026-07-22T00:00:00Z');
    const domainAgeDays = Math.floor((checkedAt - registrationDate) / (1000 * 60 * 60 * 24));
    const isConfirmedScam = matchesDomainList(hostname, riskConfig.confirmedScamDomains);
    const scanData = enforceFinalRiskConsistency({
        riskScore: isConfirmedScam ? 100 : 0,
        checks: {
            ...
            domainAnalysis: {
                status: 'danger',
                details: '舊 Shopify 商店網域到期後重新註冊，改用不同主機設施並出現同網址轉址迴圈'
            }
        }
    });

    assert.equal(domainAgeDays, 5);
    assert.equal(isConfirmedScam, true);
    assert.equal(matchesDomainList('www.shopkitchenette.com', riskConfig.confirmedScamDomains), true);
    assert.equal(matchesDomainList('shopkitchenette.com.safe.example', riskConfig.confirmedScamDomains), false);
    assert.equal(matchesDomainList('fake-shopkitchenette.com', riskConfig.confirmedScamDomains), false);
    assert.equal(matchesDomainList(hostname, whitelist), false);
    assert.equal(scanData.riskScore, 100);
    assert.ok(scanData.summaryReasons.includes('人工確認詐騙網域'));
});
...
test('Nocoding AI 大學正規課程網站不應因新網域、賺錢文案與結帳頁誤判為高風險', () => {
    ...
    const html = `
        <html lang="zh-Hant">
            <head>
                <title>立即購買｜AI 創業家 Vibe Coding 完整課程 — Nocoding AI 大學</title>
                <meta name="description" content="17 堂線上課程，從零建立自動賺錢系統，ANDY 小安老師手把手實作。">
            </head>
            <body>
                <main>
                    <h1>AI 創業家｜Vibe Coding 無代碼寫程式</h1>
                    <p>17 堂完整線上課程，包含課程內容、講師介紹、學員作品與 1 對 1 顧問支援。</p>
                    <p>由 KIDVEST ACADEMY LIMITED 越富教育科技事業有限公司提供，台灣地區由育邦文化教育科技股份有限公司代理營運。</p>
                    <p>立即購買 NT$6,980，使用 Stripe 安全付款；7 天內符合合約條件可退費。</p>
                    <a href="/course/">所有課程</a><a href="/showcase/">學員作品</a>
                    <a href="/buy/">立即購買</a><a href="/checkout/">結帳</a>
                    <a href="/terms/">服務條款</a><a href="/privacy/">隱私權政策</a>
                    <a href="https://lin.ee/xTi1XBz">LINE 聯絡</a>
                </main>
            </body>
        </html>`;
    const shoppingSignals = analyzeShoppingScamSignals({ html, url: rawUrl });
    ...
    assert.ok(riskConfig.trustedTaiwanServiceDomains.includes('noncodingai.com'));
    assert.equal(matchesDomainList(parsed.hostname, whitelist), true);
    assert.equal(isTrustedTaiwanServiceDomain(parsed.hostname), true);
    assert.equal(isVerifiedSafeRootDomain(parsed.hostname, []), true);
    assert.equal(shouldSkipAiBrandAnalysis(parsed.hostname, []), true);
    ...
    assert.ok(ecommerceSignals.categories.includes('course'));
    assert.ok(ecommerceSignals.categories.includes('cart'));
    assert.ok(ecommerceSignals.categories.includes('policy'));
    assert.equal(override.hasTrustedAllowlistOverride, true);
    assert.equal(override.riskScore, 0);
    assert.equal(isVerifiedSafeRootDomain('noncodingai.com.evil.shop', []), false);
    assert.equal(isVerifiedSafeRootDomain('fake-noncodingai.com', []), false);
    assert.match(brandApiSource, /"Nocoding AI 大學": \["noncodingai\.com"\]/);
    assert.match(brandApiSource, /"AI 創業家": \["noncodingai\.com"\]/);
    assert.equal(typeof shoppingSignals.matched, 'boolean');
});
...
test('上下游文化基金會官方連署頁不應因倡議表單誤判為高風險', () => {
    const rawUrl = 'https://agrijourney.org/petition-food-origin/';
    const parsed = new URL(rawUrl);
    const html = `
        <html lang="zh-TW">
            <head>
                <title>【連署】加工食品應標示主要原料原產地 | 上下游文化基金會</title>
                <meta name="description" content="上下游文化基金會發起加工食品應標示主要原料原產地倡議連署">
            </head>
            <body>
                <main>
                    <h1>加工食品應標示主要原料原產地</h1>
                    <p>財團法人臺中市上下游文化基金會，以獨立新聞、知識轉譯、公共倡議與體驗行動守護土地與食物。</p>
                    <p>統一編號：60842071。Email：info@agrijourney.org。電話：04-2378-3006。</p>
                    <form><input name="name" placeholder="姓名"><input name="email" placeholder="Email"><button>參與連署</button></form>
                    <a href="/about/">基金會誕生</a><a href="/mission/">使命與核心價值</a>
                    <a href="/contact/">聯絡我們</a><a href="/terms-of-use/">使用條款</a><a href="/privacy-policy/">隱私權政策</a>
                </main>
            </body>
        </html>`;
    const shoppingSignals = analyzeShoppingScamSignals({ html, url: rawUrl });
    ...
    const scanData = enforceFinalRiskConsistency({
        ...
        checks: {
            ...
            domainAnalysis: {
                status: isWhitelisted ? 'safe' : 'warning',
                details: isWhitelisted ? '受信賴台灣民營/公益服務官方網域：agrijourney.org' : '網域命名結構無明顯異常'
            }
        }
    });

    assert.ok(riskConfig.trustedTaiwanServiceDomains.includes('agrijourney.org'));
    assert.equal(isTrustedServiceRoot, true);
    assert.equal(isWhitelisted, true);
    assert.equal(shouldSkipAiBrandAnalysis(parsed.hostname), true);
    assert.equal(shoppingSignals.matched, false);
    ...
    assert.equal(scanData.riskScore, 0);
    assert.deepEqual(scanData.summaryReasons, []);
    assert.equal(isVerifiedSafeRootDomain('agrijourney.org.evil.shop'), false);
    assert.equal(isVerifiedSafeRootDomain('fake-agrijourney.org'), false);
});

test('中華磊山慈愛社官方捐款子網域不應因信用卡或收據欄位誤判為高風險', () => {
    const rawUrl = 'https://donate.ls-love.org/';
    const parsed = new URL(rawUrl);
    const whitelist = JSON.parse(fs.readFileSync(path.join(repoRoot, 'whitelist.json'), 'utf8')).domains;
    const html = `
        <html lang="zh-TW">
            <head>
                <title>讓愛看見希望｜中華磊山慈愛社線上捐款</title>
                <meta name="description" content="中華磊山慈愛社公益勸募與線上捐款平台">
            </head>
            <body>
                <main>
                    <h1>我要捐款</h1>
                    <p>讓愛看見希望。中華磊山慈愛社提供單筆捐款與定期定額公益支持。</p>
                    <p>本平台立即捐款方式為線上信用卡，亦可使用 ATM 轉帳。</p>
                    <p>立案字號：台內社字第1000109630號。勸募字號：北市社團字第1143135696號。</p>
                    <p>任何問題，請洽中華磊山慈愛社 (02)7730-7298 分機598。</p>
                    <form>
                        <input name="donor_name" placeholder="捐款芳名">
                        <input name="email" placeholder="Email">
                        <input name="receipt_title" placeholder="捐款收據資訊">
                        <button>確定捐款</button>
                    </form>
                    <a href="https://ls-love.org/">讓愛看見希望</a>
                </main>
            </body>
        </html>`;
    const shoppingSignals = analyzeShoppingScamSignals({ html, url: rawUrl });
    ...
    const isTrustedServiceRoot = isTrustedTaiwanServiceDomain(parsed.hostname);
    const isWhitelisted = isVerifiedSafeRootDomain(parsed.hostname);
    const fakeLsLove = checkBrandSimilarity('ls-love-donate.example.shop', []);
    const scanData = enforceFinalRiskConsistency({
        riskScore: isWhitelisted ? 0 : (shoppingSignals.matched ? 75 : 25),
        isTrustedAllowlist: isWhitelisted,
        checks: {
            ...
            formFields: {
                status: isWhitelisted ? 'info' : 'warning',
                details: '公益捐款頁可能要求信用卡、收據與捐款人聯絡資料'
            },
            domainAnalysis: {
                status: isWhitelisted ? 'safe' : 'warning',
                details: isWhitelisted ? '受信賴台灣公益捐款服務官方網域：ls-love.org' : '網域命名結構無明顯異常'
            }
        }
    });
    const brandApiSource = fs.readFileSync(path.join(repoRoot, 'functions/api/check-fake-brand.js'), 'utf8');

    assert.ok(riskConfig.trustedTaiwanServiceDomains.includes('ls-love.org'));
    assert.equal(matchesDomainList(parsed.hostname, whitelist), true);
    assert.equal(isTrustedServiceRoot, true);
    assert.equal(isWhitelisted, true);
    assert.equal(shouldSkipAiBrandAnalysis(parsed.hostname), true);
    assert.equal(isTrustedCoBrandCampaignHost(parsed.hostname, '中華磊山慈愛社'), true);
    assert.equal(pageBrandSignals.matched, false);
    assert.equal(fakeLsLove.matched, true);
    assert.equal(fakeLsLove.brandName, '中華磊山慈愛社');
    assert.equal(scanData.riskScore, 0);
    assert.equal(scanData.summaryReasons, undefined);
    assert.equal(isVerifiedSafeRootDomain('ls-love.org.evil.shop'), false);
...
test('新北市文化局三鶯線通車系列活動官網不應誤判為高風險', () => {
    const rawUrl = 'https://ntpcart-sy2026.com/';
    const parsed = new URL(rawUrl);
    const html = `
        <html lang="zh-TW">
            <head>
                <title>藝向新北 城市共榮｜三鶯線通車系列活動</title>
                <meta name="description" content="新北市文化局主辦，響應捷運三鶯線通車的藝文展演、市集、光雕與煙火活動">
            </head>
            <body>
                <main>
                    <h1>藝向新北 城市共榮</h1>
                    <p>三鶯線通車系列活動由新北市文化局主辦，串聯新北市美術館、陶博館與城市公共空間。</p>
                    <p>活動日期 2026/6/20 至 2026/7/12，地點為新北市美術館戶外園區。</p>
                    <p>活動包含夏至音樂節、戶外表演、光雕展演、環境聲光劇場、煙火展演與市集。</p>
                    <a href="/home/sanying-line-opening-event/">三鶯線通車慶祝活動</a>
                    <a href="/home/events/">系列活動</a><a href="/home/traffic/">交通資訊</a>
                </main>
            </body>
        </html>`;
    const shoppingSignals = analyzeShoppingScamSignals({ html, url: rawUrl });
    ...
    const scanData = enforceFinalRiskConsistency({
        ...
        checks: {
            ...
            domainAnalysis: {
                status: isWhitelisted ? 'safe' : 'warning',
                details: isWhitelisted ? '受信賴台灣文化活動官方網域：ntpcart-sy2026.com' : '網域命名結構無明顯異常'
            }
        }
    });

    assert.ok(riskConfig.trustedTaiwanServiceDomains.includes('ntpcart-sy2026.com'));
    assert.equal(isTrustedServiceRoot, true);
    assert.equal(isWhitelisted, true);
    assert.equal(shouldSkipAiBrandAnalysis(parsed.hostname), true);
    assert.equal(shoppingSignals.matched, false);
    ...
    assert.equal(scanData.riskScore, 0);
    assert.deepEqual(scanData.summaryReasons, []);
    assert.equal(isVerifiedSafeRootDomain('ntpcart-sy2026.com.evil.shop'), false);
    assert.equal(isVerifiedSafeRootDomain('fake-ntpcart-sy2026.com'), false);
});

test('全聯官方徵才履歷投遞子網域不應因表單欄位誤判為高風險', () => {
    const rawUrl = 'https://wpx.pxmartevent.com.tw/HRS/HRSLN001';
    const parsed = new URL(rawUrl);
    const html = `
        <html lang="zh-TW">
            <head>
                <title>全聯福利中心徵才投遞履歷</title>
                <meta name="description" content="全聯實業股份有限公司門市人員履歷投遞系統">
            </head>
            <body>
                <main>
                    <h1>全聯福利中心徵才投遞履歷</h1>
                    <p>全聯實業股份有限公司提供門市、物流與總部職缺履歷投遞服務。</p>
                    <p>應徵方式：請填寫履歷資料，甄選方式為面試。</p>
                    <form>
                        <input name="name" placeholder="姓名">
                        <input name="mobile" placeholder="手機">
                        <input name="email" placeholder="Email">
                        <select name="store"><option>應徵門市</option></select>
                        <textarea name="resume" placeholder="履歷"></textarea>
                        <button>送出履歷</button>
                    </form>
                    <a href="https://hr.pxmart.com.tw/career.html">全聯人才招募網</a>
                </main>
            </body>
        </html>`;
    const shoppingSignals = analyzeShoppingScamSignals({ html, url: rawUrl });
    ...
    const scanData = enforceFinalRiskConsistency({
        ...
        checks: {
            ...
            domainAnalysis: {
                status: isWhitelisted ? 'safe' : 'warning',
                details: isWhitelisted ? '受信賴全聯官方徵才服務網域：pxmartevent.com.tw' : '網域命名結構無明顯異常'
            }
        }
    });

    assert.ok(riskConfig.trustedTaiwanServiceDomains.includes('pxmartevent.com.tw'));
    assert.equal(isTrustedServiceRoot, true);
    assert.equal(isWhitelisted, true);
    assert.equal(shouldSkipAiBrandAnalysis(parsed.hostname), true);
    assert.equal(shoppingSignals.matched, false);
    ...
    assert.equal(scanData.riskScore, 0);
    assert.deepEqual(scanData.summaryReasons, []);
    assert.equal(isVerifiedSafeRootDomain('pxmartevent.com.tw.evil.shop'), false);
    assert.equal(isVerifiedSafeRootDomain('fake-pxmartevent.com.tw'), false);
});
...
test('食藥署膨風廣告同步腳本可抽出涉嫌違規網址並產生官方警示索引', async () => {
    const syncModule = await import(pathToFileURL(path.join(repoRoot, 'scripts/sync-fda-puffery-ads.mjs')).href);
    const sampleListHtml = `
        <tr><td class='alignCenter'>1</td><td>
            <a href="newsContent.aspx?cid=5085&id=115P9999&type=pmds" title="國外網站(https://bad-health.example.tw/sale)涉嫌違規廣告產品：測試膠囊">國外網站(https://bad-health.example.tw/sale)涉嫌違規廣告產品：測試膠囊</a>
        </td><td class='alignCenter'>2026-06-29</td>
    `;
    const sourceUrl = 'https://www.fda.gov.tw/TC/newsContent.aspx?cid=5085&id=115P9999&type=pmds';
    const sampleDetailHtml = `
        <span class="fdtitle">國外網站涉嫌違規廣告產品：測試膠囊</span>
        <span class="orangeText">| 發布日期：2026-06-29　| 維護日期：2026-06-30</span>
        <span class="whiteText">發布單位：新北市政府衛生局</span>
        <table class="pmds"><tbody>
            <tr><td class="title">網站名稱</td><td>測試網站</td></tr>
            <tr><td class="title">產品名稱</td><td>測試膠囊</td></tr>
            <tr><td class="title">產品類別</td><td>食品</td></tr>
            <tr><td class="title">監控日期</td><td>2026/06/20</td></tr>
            <tr><td class="title">涉嫌違規網址</td><td>https://bad-health.example.tw/sale?a=1</td></tr>
            <tr><td class="title">涉嫌違規宣稱詞句</td><td>宣稱改善慢性病與治療效果</td></tr>
            <tr><td class="title">涉嫌違規態樣</td><td>違反食品安全衛生管理法第28條規定</td></tr>
            <tr><td class="title">建議消費者</td><td>上述廣告涉嫌違規，提醒消費者勿信勿購買。</td></tr>
        </tbody></table>
    `;
    const rows = syncModule.parseFdaNewsRows(sampleListHtml);
    const detail = syncModule.parseFdaPmdsDetail(sampleDetailHtml);
    const records = syncModule.toFdaOfficialAlertRecords(rows, new Map([[sourceUrl, detail]]));

    assert.equal(rows.length, 1);
    assert.equal(rows[0].sourceUrl, sourceUrl);
    assert.equal(detail.fields['產品名稱'], '測試膠囊');
    assert.equal(detail.fields['涉嫌違規網址'], 'https://bad-health.example.tw/sale?a=1');
    assert.deepEqual(syncModule.extractUrlsFromText(`${rows[0].title} ${detail.fields['涉嫌違規網址']}`), [
        'https://bad-health.example.tw/sale',
    ...
    assert.equal(records[0].productName, '測試膠囊');
    assert.equal(records[0].rootDomain, 'example.tw');
    assert.deepEqual(records[0].rootDomains, ['example.tw']);
    assert.equal(records[0].publishedDate, '2026-06-29');
    assert.equal(records[0].monitoredDate, '2026-06-20');
    assert.match(records[0].warning, /勿信勿購買/);
    assert.match(records[0].claimSummary, /治療效果/);
});
